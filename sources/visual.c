...
static t_map	*vs_create(void)
{
	...
	if (!(map->window = mlx_new_window(map->mlx, W_WIDTH, W_HEIGHT, "Filler")))
		ft_error("couldn't init");
...
int				main(void)
{
	...
	mlx_hook(map->window, 2, 0, vs_rep_key, map);
	mlx_hook(map->window, 4, 0, hook_mousedown, map);
	mlx_hook(map->window, 5, 0, hook_mouseup, map);
